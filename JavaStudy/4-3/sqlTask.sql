...
-- [回答]
 SELECT store_name , store_code FROM store_table ORDER BY store_name ASC
 
-- 3. 在庫テーブルに店舗テーブル、商品テーブルを「内部結合」し、店舗名・商品名・在庫数を全て取得しなさい。
-- [回答]
 SELECT * FROM stock_table JOIN store_table ON stock_table.store_code = store_table.store_code
 SELECT * FROM stock_table JOIN goods_table ON stock_table.goods_code = stock_table.goods_code
 SELECT  store_name , goods_name , quantitiy FROM stock_table
-- 4. 商品テーブルから全商品の単価の平均値を抽出しなさい。
-- [回答]
 SELECT AVG(price) FROM goods_code
 
-- 5. 店舗コード='EA01'の在庫数の平均値より大きい在庫数を持つ店舗コードを抽出しなさい。
-- [回答]
 SELECT AVG(quantitiy) FROM EA01 > 
